/**
 * Shell ask-gate for the "带Shell鉴权的标准模式" preset.
 *
 ...
 * the shell tools (`bash`, `pwsh`), so each shell call is routed through the
 * host approval service and executes only after the user approves it
 * (`allowed-once`); otherwise it is denied. Every other tool call passes
 * through untouched.
 *
